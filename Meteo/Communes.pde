...
List<Commune> rechercheVilles(String nom){
  ...
  for(int rang=0; rang<max; rang ++) {
    float ressemblance = compareString(normaliserNom(tableCommunes.getString(rang,1)), nom);
    String[] alt = tableCommunes.getString(rang, 3).split(",");
    for(String s : alt)
      ressemblance = Math.max(ressemblance, compareString(normaliserNom(s), nom));
...
String normaliserNom(String nomCommune) {
  ...
  if(ret.startsWith("st ") || ret.startsWith("st."))
      ret = "saint " + ret.substring(3);
  return ret;
